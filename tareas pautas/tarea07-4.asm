...
variables:                              ; declaracion de variables.
    x_add_1 dw 0                        ; variable que se modifica a medida que se inc/dec la coordenada x.    
    y_add_1 dw 0                        ; variable que se modifica a medida que se inc/dec la coordenada y.
    x_0 dw 0                            ; coordenada x del punto inicial.
    y_0 dw 0                            ; coordenada y del punto inicial.
    x_1 dw 0                            ; coordenada x del punto final.
    y_1 dw 0                            ; coordenada y del punto final.
    delta_x dw 0                        ; delta_x = x_1 - x_0.
    delta_y dw 0                        ; delta_y = y_1 - y_0.
    dos_delta_x dw 0                    ; 2*delta_x
    dos_delta_y dw 0                    ; 2*delta_y
    dos_delta_y_menos_dos_delta_x dw 0  ; 2*delta_y - 2*delta_x.
    k dw 0                              ; k (cantidad de pasos) = delta_x, si delta_x == 0 se iguala a delta_y.
    p dw 0                              ; p = 2*delta_y - 2*delta_x.
    dos dw 2                            ; variable 2.
    
    vector_inicio dw 0,0,14,7,10,9     ; vector inicial.
    vector_termino dw 14,7,10,9,0,0    ; vector termino.
    estrellas dw 3                     ; cantidad de estrellas.

;    vector_inicio dw  0,0 ;300,180,319,200,319,180
;    vector_termino dw 10,11 ;321,201,319,180,300,180
;    estrellas dw 1
